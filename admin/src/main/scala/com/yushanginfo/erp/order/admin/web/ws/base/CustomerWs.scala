...
import com.yushanginfo.erp.base.model.{Customer, Product}
...
class CustomerWs extends ActionSupport {
  var entityDao: EntityDao = _

  @response
  @mapping("")
  def index: Seq[Properties] = {
    val query = OqlBuilder.from(classOf[Customer], "es")
    get("q") foreach { q =>
      if (Strings.isNotEmpty(q)) {
        query.where("es.code like :q or es.quickCode like :q or es.name like :q", "%" + q + "%")
      }
    }
